    ...
    if ($url[0] == 'users') {
      ...
      if (!isset($url[1])) {
        ...
        $response = $sales->create(
          ...
          isset($request->attachment) ? ($request->attachment != null ? $request->attachment : null) : null
        );
      ...
      } else if ($url[1] == 'attachment') {
        if (isset($url[2]) && $url[2] == 'delete') {
          $response = $sales->delete_attachment_by_sale_id(
            isset($url[3]) ? (int) $url[3] : null
          );

          if ($response) {
            $api->generate_user_log(
              $api->user_id,
              'sales.attachment.delete',
              json_encode($response)
            );
            http_response_code(204);
          } else {
